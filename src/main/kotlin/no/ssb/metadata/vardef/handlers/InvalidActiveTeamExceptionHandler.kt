...
@Singleton
@Requirements(
    Requires(classes = [InvalidActiveTeamException::class, ExceptionHandler::class]),
)
class InvalidActiveTeamExceptionHandler(
    private val errorResponseProcessor: ErrorResponseProcessor<*>,
) : ExceptionHandler<InvalidActiveTeamException, HttpResponse<*>> {
    override fun handle(
        request: HttpRequest<*>,
        exception: InvalidActiveTeamException,
    ): HttpResponse<*> =
