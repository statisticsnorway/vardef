...
class VardefTokenValidatorTest {
    ...
    fun `verify token comes from dapla lab`(){
        val result: Publisher<JWT> = vardefTokenValidator.validate(JwtTokenHelper.jwtTokenSigned().parsedString, HttpRequest.POST("/vardef",""))

        result.subscribe(object : Subscriber<JWT> {
