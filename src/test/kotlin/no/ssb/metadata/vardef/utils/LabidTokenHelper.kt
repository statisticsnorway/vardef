...
class LabIdTokenHelper {
    companion object {
        fun labIdTokenSigned(
            audienceClaim: List<String> = listOf(
                "onyxia-api",
                "broker",
                "account",
            ),
            activeGroup: String = TEST_DEVELOPERS_GROUP,
            daplaGroups: List<String>? = listOf(
            ...
            if (includeUsername) {claims.put("sub", LABID_TEST_USER)}
            if (includeActiveGroup) {claims.put("dapla.group", activeGroup)}
        ...
        private val header = Header.parse(
            """
            {
              "alg": "RS256",
              "typ": "JWT",
        ...
        private val signature = Base64URL(
