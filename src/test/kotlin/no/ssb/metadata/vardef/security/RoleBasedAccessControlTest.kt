...
class RoleBasedAccessControlTest : BaseVardefTest() {
    ...
    fun `create variable definition active group not developers`(
        ...
        spec
            ...
            .auth()
            .oauth2(
                JwtTokenHelper.jwtTokenSigned(daplaGroups = listOf(group), activeGroup = group).parsedString,
            ).`when`()
            .request(method, path)
            .then()
            .statusCode(HttpStatus.FORBIDDEN.code)
    ...
    fun `request where active group is valid but not owner`(
        ...
        spec
            ...
            .auth()
            .oauth2(
                JwtTokenHelper.jwtTokenSigned(activeGroup = "play-foeniks-a-developers").parsedString,
            ).`when`()
            .request(method, path)
            .then()
            .statusCode(HttpStatus.FORBIDDEN.code)
