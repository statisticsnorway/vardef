...
class KlassServiceTest {
    ...
    fun `get klass code item test`() {
        klassService
            .getCodeItemFor("702", "01", no.ssb.metadata.vardef.models.SupportedLanguages.NB)
            ?.let { klassItem ->
        ...
        klassService
            .getCodeItemFor("702", "41", no.ssb.metadata.vardef.models.SupportedLanguages.NB)
            ?.let { klassItem ->
                assertThat(klassItem).isEqualTo(null)
            }

        klassService
            .getCodeItemFor("702", "01", no.ssb.metadata.vardef.models.SupportedLanguages.NN)
            ?.let { klassItem ->
                assertThat(klassItem.referenceUri).isEqualTo("https://data.ssb.no/api/klass/v1/classifications/702/")
                assertThat(klassItem.code).isEqualTo("01")
