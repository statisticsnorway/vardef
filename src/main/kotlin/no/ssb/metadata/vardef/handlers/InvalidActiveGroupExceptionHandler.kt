...
@Singleton
@Requirements(
    Requires(classes = [InvalidActiveGroupException::class, ExceptionHandler::class]),
)
class InvalidActiveGroupExceptionHandler(
    private val errorResponseProcessor: ErrorResponseProcessor<*>,
) : ExceptionHandler<InvalidActiveGroupException, HttpResponse<*>> {
    override fun handle(
        request: HttpRequest<*>,
        exception: InvalidActiveGroupException,
    ): HttpResponse<*> =
